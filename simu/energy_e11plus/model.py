"""Power and electric-energy prediction for the E1-1plus AC simulator.

This module mirrors :mod:`simu.energy.model` so the
``AirConditionerSimulator`` can be reconfigured by simply importing
``EnergyModel`` from this package instead.  The ``predict_power`` /
``energy_kwh`` / ``reset`` / low-frequency-correction APIs are byte-for-byte
compatible with the original implementation; the only difference is the
default model path, which points to ``simu/energy_e11plus/per_mode_models.pkl``.

The bundled pickle was trained from the GBK-encoded status CSVs under
``data/E1-1plus`` (see :mod:`simu.energy_e11plus.train`).  Features are
``[freq, eev, fan_out]``; the target is instantaneous power in watts derived
from the cumulative ``energy_cum`` column.

Heads-up on heating: the E1-1plus dataset contains only 制冷 (cool) logs,
so ``mode="制热"`` reuses the cool-mode regressor as a fallback.  Collect
制热 runs and rerun ``simu.energy_e11plus.train`` to get a heating-specific
model.
"""
...
DEFAULT_MODEL_PATH = PROJECT_DIR / "simu" / "energy_e11plus" / "per_mode_models.pkl"
