"""V4 hybrid virtual-room dynamics.
...
class V4Config:
    ...
    cooling_tau_min_seconds: float = 10.0
    cooling_tau_max_seconds: float = 600.0
...
class HybridRoomV4(nn.Module):
    ...
    def step_state(self, state: dict[str, torch.Tensor], control: torch.Tensor) -> dict:
        normalized_control = (control - self.control_mean) / self.control_scale
        temperature = state["temperature"]
        initial_temperature = state["initial_temperature"]
        outdoor_temperature = state["outdoor_temperature"]
        dynamic = torch.cat([
            normalized_control,
            ((temperature - initial_temperature) / 8.0).unsqueeze(-1),
            ((outdoor_temperature - temperature) / 20.0).unsqueeze(-1),
        ...
        compressor_gate = torch.sigmoid((control[:, 0] - 1.0) / 1.5)
        ...
        load = self.deterministic_load(
            temperature, outdoor_temperature, state["load_coefficient"],
        )
